...
function fetchProjects() {
  ...
  const fields = ["ID", "Title", "phases", "members", "status", "key_IDs", "months", "years", "note_types"];
  getItem("Projects", fields)
    .then((res) => {
...
function showAdd(colIdx) {
  ...
  if (current.value.columns[colIdx].title === "status") {
...
function addElement(colIdx) {
  ...
  if (value) {
    ...
    // For status column, combine text and color
    let finalValue = value;
    if (current.value.columns[colIdx].title === "status") {
      finalValue = `${value}#${newStatusColor.value.replace("#", "")}`;
    }
...
function confirmDeleteKeyRow(rowIdx) {
  ...
  projectStore.setLoading(true);
  deleteItem("Keys", [keyColObj.items[rowIdx]])
    .then((res) => {
      const currentValue = projectStore.currentProject["key_IDs"];
      const parts = typeof currentValue === "string" ? currentValue.split(",") : [];
...
function updateStatusColor(colIdx, itemIdx, newColor) {
  const keyName = Object.keys(projectStore.currentProject)[colIdx + 2];
  const currentValue = Object.values(projectStore.currentProject)[colIdx + 2];
  const parts = typeof currentValue === "string" ? currentValue.split(",") : [];
  const itemToUpdate = parts[itemIdx];
  const newItemValue = `${itemToUpdate.split("#")[0]}#${newColor.replace("#", "")}`;
  parts[itemIdx] = newItemValue;
  const valueName = parts.join(",");
...
<template>
  <LoadingSpinner :showing="projectStore.loading" text="Loading...">
    <div class="project-controls">
      ...
      <select
        id="project-select"
        :value="projectStore.currentProject ? projectStore.currentProject.Title : 'No projects'"
        @change="selectProject"
        class="project-select"
      >
        <option v-for="project in projectStore.setups" :key="project.ID" :value="project.Title">
          {{ project.Title }}
        </option>
      </select>
        ...
        <ul v-if="column.title !== 'KEY'" class="dashboard-list">
          <li v-for="(item, i) in column.items" :key="i" class="dashboard-list-item">
            ...
            <div v-else class="status-item-container">
              <input
                :value="item.split('#')[1] ? '#' + item.split('#')[1] : '#666'"
                @change="updateStatusColor(colIdx, i, $event.target.value)"
                class="status-color-picker"
                type="color"
                title="Change color"
              />
              <q-badge
                :style="{ backgroundColor: item.split('#')[1] ? '#' + item.split('#')[1] : '#666' }"
                class="status-badge"
              >
                {{ item.split("#")[0] || item }}
              </q-badge>
          ...
          <li v-if="showAddInput[colIdx]" class="dashboard-list-item add-item">
            <div class="add-input-container">
              ...
              <div class="inline-buttons">
                <button class="inline-confirm-btn" @click="addElement(colIdx)">Add</button>
        ...
        <div v-else class="dashboard-list key-list">
          <div v-for="(row, rowIdx) in currentKeyRows" :key="'row-' + rowIdx" class="key-row">
            ...
            <div class="delete-area">
              ...
              <template v-else>
                <button class="delete-btn" @click="showDeleteConfirm(colIdx, rowIdx)" title="Delete">❌</button>
              </template>
            </div>
          </div>
